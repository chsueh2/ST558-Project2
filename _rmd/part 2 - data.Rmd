...
- `vtable`: summary statistics about variables in data frames
- `ggcorrplot`: correlation plot matrix
- `Hmisc`: for its quick histogram plotting function `hist.data.frame()`
...
pacman::p_load(
  ...
  tidyverse,
  lubridate,
  glue, scales, skimr, vtable,
  ggcorrplot, Hmisc,
  caret, randomForest, gbm
)
```
...
Next we subset data by channel. This is done by specifying rmarkdown parameter `channel`. By default, its value is `all` to keep all data  without any subsetting.

```{r}
# subset by channel
if(params$channel != "all"){
  df_train <- df_train %>% filter(channel == params$channel)
  df_test <- df_test %>% filter(channel == params$channel)
}
```
