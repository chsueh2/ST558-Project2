...
# Model A: `weekday` (categorical) + selected numeric predictors
fit_lm_A <- fit_model(
  shares ~ ., df_train[, vars_A], df_test[, vars_A], method = "lm",
  trControl = trainControl(method = "cv", number = 5))

# Model B: `is_weekend` (categorical) + all numeric predictors
fit_lm_B <- fit_model(
  shares ~ ., df_train[, vars_B], df_test[, vars_B], method = "lm",
  trControl = trainControl(method = "cv", number = 5))
```

### Random Forests Model

Random forests is also known as random decision forests model. It is an ensemble method based on decision trees. It uses bagging to create multiple trees from bootstrap samples and aggregate the results to make decisions. However, instead of all predictors, only a subset of the predictors are used to grow trees. This effectively prevents highly correlated trees if there exists a strong predictor. Again, 5-fold cross validation is used to choose the best model. Note that:

- We can include hyperparameter tuning, for example `tuneGrid = expand.grid(mtry = 1:20)`
- In addition to `randomForest` package, we also use `ranger` package and tune the following parameters:
  - mtry: number of variables randomly sampled as candidates at each split
  - splitrule: since this is for regression, we choose "variance" option to use variance as our splitting rule
...
# Model A: `weekday` (categorical) + selected numeric predictors
# fit_rf_A <- fit_model(
#   shares ~ ., df_train[, vars_A], df_test[, vars_A], method = "rf",
#   trControl = trainControl(method = "cv", number = 5),
#   tuneGrid = expand.grid(mtry = 1:16))
# 
# # Model B: `is_weekend` (categorical) + all numeric predictors
# fit_rf_B <- fit_model(
#   shares ~ ., df_train[, vars_B], df_test[, vars_B], method = "rf",
#   trControl = trainControl(method = "cv", number = 5),
#   tuneGrid = expand.grid(mtry = 1:16))
# 
...
#   shares ~ ., df_train[, vars_A], df_test[, vars_A], method = "ranger",
#   trControl = trainControl(method = "cv", number = 5),
...
#   shares ~ ., df_train[, vars_B], df_test[, vars_B], method = "ranger",
#   trControl = trainControl(method = "cv", number = 5),
#   tuneGrid = expand.grid(
#     mtry = 1:16,
#     splitrule = "variance",
...
#   shares ~ ., df_train[, vars_A], df_test[, vars_A], method = "gbm",
#   trControl = trainControl(method = "cv", number = 5),
#   tuneGrid = expand.grid(
#     n.trees = seq(5, 200, 5),
...
#   shares ~ ., df_train[, vars_B], df_test[, vars_B], method = "gbm",
#   trControl = trainControl(method = "cv", number = 5),
#   tuneGrid = expand.grid(
#     n.trees = seq(5, 200, 5),
#     interaction.depth = 1:4,
