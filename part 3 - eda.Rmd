...
Next, lets plot a histogram of the `shares` data so that we can visually see it's distribution. For example, we can determine if the distribution or `shares` is symmetric, skewed left, or right.
...
g + geom_histogram(bins=K) +
  labs(title = "Histogram: Distribution of Shares",y="Frequency",x="Shares")
```
...
g + geom_boxplot(fill = "grey") +
  labs(x="Number of Shares >= Mean (0=no,1=yes)",title="Boxplot: Are the Number of Shares Greater than the Mean?",y="Count")
```
...
ggcorrplot(r, 
            ...
            type = "lower",
            tl.cex = 6)
#for our models, we use a correlation coefficient of >0.75 and <-0.75
```
